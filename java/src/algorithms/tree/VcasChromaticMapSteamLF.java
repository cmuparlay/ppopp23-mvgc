...
public class VcasChromaticMapSteamLF<K extends Comparable<? super K>,V> {
    ...
    public static long[] previousScanTime = new long[ThreadID.MAX_THREADS*Epoch.PADDING]; // nanoseconds
    ...
    public boolean getBaseline() {
        ...
        /**
     * Compacts all the version lists 
     */
    public void compactAll() {
        AnnScan scan = AnnScan.get();
        CameraSteamLF.scanAnnouncements(scan);

        HashSet<Node> visited = new HashSet<Node>();
        compactAll(root, visited);
    }

    private void compactAll(final Node node, HashSet<Node> visited) {
        if(node == null || visited.contains(node)) return;
        visited.add(node);
        node.compactSequential();

        compactAll(node.nextv, visited);
        if(node instanceof InternalNode) {
            compactAll(((InternalNode)node).getLeft(), visited);
            compactAll(((InternalNode)node).getRight(), visited);           
        }
    }
    
    public VcasChromaticMapSteamLF() {
        this(DEFAULT_d);
    }

    public VcasChromaticMapSteamLF(final int allowedViolationsPerPath) {
...
//                assert olderVersion.right == node;
                ...
                stats.add(countNodes((Node) olderVersion, countVersionList, visited));
            }
        }
    ...
    public final void versionListLengths(final Node node, HashSet<Node> visited, ArrayList<Integer> lengths) {
        ...
        visited.add(node);
        versionListLengths(node.nextv, visited, lengths);
        if(node instanceof InternalNode) {
    ...
    public final int nodesInReachableVersionLists() {
        ...
        for(Node node : reachable)
            numNodes += 1 + nodesInVersionList(node.nextv, visited);
        return numNodes; }

    public final int listLength(final Node node) {
        if(node == null) return 0;
        return 1 + listLength(node.nextv);
    }

    public final int nodesInVersionList(final Node node, HashSet<Node> visited) {
        if(node == null) return 0;
        return nodesInSubtree(node, visited) + nodesInVersionList(node.nextv, visited);
    }
    ...
    public static abstract class Node {
        ...
        public void initTS() {
          if(ts == TBD) {
              long curTS = 0;
              if(Camera.INCREMENT_ON_UPDATE) curTS = CameraSteamLF.takeSnapshot()+1;
              else curTS = CameraSteamLF.getTimestamp();
              tsUpdater.compareAndSet(this, TBD, curTS);
          }
        }
        public void compactSequential() {
            Node cur = this;
            AnnScan annScan = CameraSteamLF.globalAnnScan.get(Epoch.PADDING);

            long[] sortedAnn = annScan.reserved;
            long scanTS = annScan.ts;
            ...
            while(cur != null) {
                ...
                if (curTS > scanTS) {
                    ...
                    if (i >= 0 && sortedAnn[i] >= next.ts) { // next needed
                        ...
                        Node newNext = next.nextv;
                        while (newNext != null && (i < 0 || sortedAnn[i] < newNext.ts))
                            newNext = newNext.nextv;
                        while (!nextvUpdater.compareAndSet(cur, next, newNext)) {
                            next = cur.nextv;
    ...
    // compact node.left iff left == true
    public static void compact(InternalNode node, boolean left) {
        AnnScan scan = AnnScan.get();
        // rescan announcement array if out of date
        long curTime = System.nanoTime();
        int tid = ThreadID.threadID.get();
        if(curTime > previousScanTime[tid*Epoch.PADDING] + 1000000) {
            previousScanTime[tid*Epoch.PADDING] = curTime;
            CameraSteamLF.scanAnnouncements(scan);
        }

        AnnScan annScan = CameraSteamLF.globalAnnScan.get(Epoch.PADDING);
        Node cur = (left ? node.left : node.right);
        AnnScan annScan2 = CameraSteamLF.globalAnnScan.get(Epoch.PADDING);

        while(annScan != annScan2) {
            annScan = annScan2;
            cur = (left ? node.left : node.right);
            annScan2 = CameraSteamLF.globalAnnScan.get(Epoch.PADDING);
        }
        cur.initTS();
        ...
        while(cur != null) {
            ...
            if (curTS > scanTS) {
                ...
                if (i >= 0 && sortedAnn[i] >= next.ts) { // next needed
                    ...
                    while (newNext != null && (i < 0 || sortedAnn[i] < newNext.ts))
                        newNext = newNext.nextv;
                    while (!Node.nextvUpdater.compareAndSet(cur, next, newNext)) {
                        next = cur.nextv;
                        long newNextTS = (newNext == null ? -1 : newNext.ts);
                        long nextTS = (next == null ? -1 : next.ts);
                        if (nextTS <= newNextTS) break;
    ...
    */
    public final Object[] rangeScan(final K a, final K b) {
        long ts = 0;
        if(Camera.INCREMENT_ON_UPDATE) ts = CameraSteamLF.getTimestamp();
        else ts = CameraSteamLF.takeSnapshot();
        ...
        if(!isBaseline) CameraSteamLF.unreserve();
