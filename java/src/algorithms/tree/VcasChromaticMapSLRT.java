...
public class VcasChromaticMapSLRT<K extends Comparable<? super K>,V> {
    ...
    public VcasChromaticMapSLRT() {
        this(DEFAULT_d);
    }

    public VcasChromaticMapSLRT(final int allowedViolationsPerPath) {
    ...
    public static void cleanup() {
        for(RangeTrackerSLRT.Range range : (List<RangeTrackerSLRT.Range>) CameraSLRT.rt.ejectAll()) {
            if(range.left) ((InternalNode) range.t).left.nextv = null;
            else           ((InternalNode) range.t).right.nextv = null;
        }
    }
...
//            numRemoves[i*Epoch.PADDING] = 0;
    ...
    public final NodeStats countNodes(final boolean countVersionList) {
        ...
        return countNodes(root.left, countVersionList, visited);
    }
...
//            assert this.doublyLinkedVsSinglyLinked() == 0;
    ...
    public static abstract class Node {
        ...
        public volatile Node nextv;
        public volatile long ts;

        public Node(final int weight) {
            nextv = dummyNextv; // allows multiple processes to tryAppend with the same B and C
            this.ts = TBD;
            this.weight = weight;
        }
        ...
        public void compact(AnnScan ar) {
            this.initTS();
            AnnScan annScan = (AnnScan) CameraSLRT.camera.rt.globalAnnScan.get(Epoch.PADDING);
            Node cur = this;
            AnnScan annScan2 = (AnnScan) CameraSLRT.camera.rt.globalAnnScan.get(Epoch.PADDING);

            while(annScan != annScan2) {
                annScan = annScan2;
                cur = this;
                annScan2 = (AnnScan) CameraSLRT.camera.rt.globalAnnScan.get(Epoch.PADDING);
            }
        ...
        public void compactSimple() {
            long curTS = this.ts;
            final Node oldNext = this.nextv;
            Node newNext = oldNext;
...
//            nextvUpdater.compareAndSet(newV, dummyNextv, oldV);
            ...
            nextvUpdater.compareAndSet(newV, Node.dummyNextv, head);
            if(updateLeft.compareAndSet(this, head, newV)) {
                newV.initTS();
                if(head != null) {
                    long headts = head.ts;
                    long newts = newV.ts;
                   if(headts == newts) newV.compactSimple();
                   else {
                        CameraSLRT.rt.deprecate(CameraSLRT.camera, this, true, headts, newts);
                   }
                }
                return true;
            } else {
            ...
            if(updateRight.compareAndSet(this, head, newV)) {
                ...
                if(head != null) {
                   ...
                   if(headts == newts) newV.compactSimple();
                        ...
                        CameraSLRT.rt.deprecate(CameraSLRT.camera, this, false, headts, newts);
                   }
                }
                return true;
            } else {
