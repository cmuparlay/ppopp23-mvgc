...
import algorithms.common.DoublyLinkedListBBF;
...
public class VcasChromaticMapBBF<K extends Comparable<? super K>,V> {
    ...
    public VcasChromaticMapBBF() {
        this(DEFAULT_d);
    }

    public VcasChromaticMapBBF(final int allowedViolationsPerPath) {
    ...
    public static void cleanup() {
        for(Node node : (List<Node>) CameraBBF.rt.ejectAll()) {
            DoublyLinkedListBBF.remove(node);
        }
    }
    ...
    public final NodeStats countNodes(final Node node, final boolean countVersionList, final boolean useBackPointers, HashSet<Node> visited) {
        ...
        if(countVersionList) {
            DoublyLinkedListBBF.Node olderVersion = DoublyLinkedListBBF.getOlderVersion(node);
            ...
            if(useBackPointers) {
                stats.add(countNodes((Node) ((DoublyLinkedListBBF.Node) node).right, countVersionList, useBackPointers, visited));
            }
        }
    ...
    public final void versionListLengths(final Node node, HashSet<Node> visited, ArrayList<Integer> lengths) {
        ...
        visited.add(node);
        versionListLengths((Node) DoublyLinkedListBBF.getOlderVersion(node), visited, lengths);
    ...
    public final int nodesInReachableVersionLists() {
        ...
        for(Node node : reachable)
            numNodes += 1 + nodesInVersionList((Node) DoublyLinkedListBBF.getOlderVersion(node), visited);
        return numNodes; }

    public final int listLength(final Node node) {
        if(node == null) return 0;
        return 1 + listLength((Node) DoublyLinkedListBBF.getOlderVersion(node));
    }

    public final int nodesInVersionList(final Node node, HashSet<Node> visited) {
        if(node == null) return 0;
        return nodesInSubtree(node, visited) + nodesInVersionList((Node) DoublyLinkedListBBF.getOlderVersion(node), visited);
    }
    ...
    public static abstract class Node extends DoublyLinkedListBBF.Node {
        ...
        public void initTS() {
            if (ts == TBD) {
                ...
                super.tsUpdater.compareAndSet(this, TBD, curTS);
            }
        }
    ...
    public static final class InternalNode extends Node {
        ...
        public InternalNode(final Comparable key, final int weight, final Node left, final Node right) {
            ...
            this.op = dummy;
            if(left != null) {
                if(left.left == dummyNextv) {
                    DoublyLinkedListBBF.tryAppend(updateLeft, this, null, left); // guaranteed to succeed
                    left.initTS(); // TODO: can be optimized to avoid reading global timestamp
                } else this.left = left;
            }
            if(right != null) {
                if(right.left == dummyNextv) {
                    DoublyLinkedListBBF.tryAppend(updateRight, this, null, right); // guaranteed to succeed
        ...
        Node getLeft(long ts) {
            ...
            node.initTS();
            return (Node) DoublyLinkedListBBF.find(node, ts);
        }
...
//            nextvUpdater.compareAndSet(newV, dummyNextv, oldV);
            ...
            if(DoublyLinkedListBBF.tryAppend(updateLeft, this, head, newV)) {
                newV.initTS();
                if(head != null) {
                    long headts = head.ts;
                    long newts = newV.ts;
                    if(headts == newts) DoublyLinkedListBBF.remove(head);
                    else {
...
//            nextvUpdater.compareAndSet(newV, dummyNextv, oldV);
            ...
            if(DoublyLinkedListBBF.tryAppend(updateRight, this, head, newV)) {
                newV.initTS();
                if(head != null) {
                    long headts = head.ts;
                    long newts = newV.ts;
