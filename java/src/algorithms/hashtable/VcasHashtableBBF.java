...
public class VcasHashtableBBF<K extends Comparable<? super K>,V> {
    ...
    public static class HeadNode<K extends Comparable<? super K>,V> extends DoublyLinkedListBBF.Node {
        public K key;
        public V value;
...
//        assert head.leftv != HeadNode.dummyNextv;
        while(head != null && head.ts > ts) {
            head = (HeadNode) DoublyLinkedListBBF.getOlderVersion(head);
            traversedNodes++;
...
//            System.out.println("head.ts: " + head.ts);
    ...
    public boolean bucketCompareAndSet(int idx, HeadNode<K,V> oldHead, HeadNode<K,V> newHead) {
        ...
        if(DoublyLinkedListBBF.tryAppend(buckets, idx, oldHead, newHead)) {
            newHead.initTS();
//            System.out.println(newHead.ts);
    ...
    public VcasHashtableBBF() {
        this(DEFAULT_SIZE);
    }

    public VcasHashtableBBF(int size) {
...
//            System.out.println(i + " " + getVersion(i, ts));
    ...
    private int versionListLength(HeadNode node) {
        ...
        while(node != null) {
            count++;
            node = (HeadNode) DoublyLinkedListBBF.getOlderVersion(node);
        }
        return count;
    }
