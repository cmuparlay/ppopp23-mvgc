...
public class VcasHashtableEpoch<K extends Comparable<? super K>,V> {

    private static final int DEFAULT_SIZE = 1000000;
    public static final Epoch<HeadNode> epoch = new Epoch();
    ...
    public static class HeadNode<K extends Comparable<? super K>,V> extends Reclaimable {
        public K key;
        public V value;
        ...
        public void initTS() {
            if(ts == TBD) {
                long curTS = 0;
                if(Camera.INCREMENT_ON_UPDATE) curTS = Camera.takeSnapshot()+1;
                else curTS = Camera.getTimestamp();
...
//            System.out.println("head.ts: " + head.ts);
    ...
    public boolean bucketCompareAndSet(int idx, HeadNode<K,V> oldHead, HeadNode<K,V> newHead) {
        ...
        if(head != null) {
            head.initTS();
            HeadNode<K,V> headNext = head.nextv;
...
//            System.out.println(newHead.ts);
            if(oldHead != null && newHead.ts == oldHead.ts)
                newHead.nextv = oldHead.nextv;
            if(newHead.nextv != null)
                epoch.retire(newHead);
            return true;
        } else {
    ...
    public VcasHashtableEpoch() {
        this(DEFAULT_SIZE);
    }

    public VcasHashtableEpoch(int size) {
        num_buckets = size;
...
//            buckets.set(i, new HeadNode<>(null, null, null));
    ...
    public int hash(Integer key) {
        return (Hash32.hash(key) & 0x7fffffff);
    }
    ...
    public Object[] rangeScan(final K a, final K b) {
        ...
        epoch.announce();
        long ts = 0;
        if(Camera.INCREMENT_ON_UPDATE) ts = Camera.getTimestamp();
        else ts = Camera.takeSnapshot();
...
//            System.out.println(i + " " + getVersion(i, ts));
        ...
        if(!isBaseline) epoch.unannounce();
