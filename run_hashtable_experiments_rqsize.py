...
if len(sys.argv) == 1 or sys.argv[1] == '-h':
  print("Usage: python3 run_hashtable_experiments_rqsize.py [num_keys] [update_threads] [rq_threads] [small_rq_threads] [rqsizelist] [outputfile] [num_repeats] [runtime] [JVM memory size] [zipf]")
  print("For example: python3 run_hashtable_experiments_rqsize.py 10000 4 4 0 [8,256] graph.png 1 1 1G 0")
  exit(0)
...
graphtitle = "hashtable-" + str(num_keys) + "keys-" + str(update_threads) + "up-" + str(rq_threads) + "rq-" + str(small_rq_threads) + "smallrq-" + JVM_mem_size + inc_update
if zipf == 0:
  graphtitle += "-uniform"
else:
...
graphtitle = graphtitle.replace('-64G','')
...
datastructures = ["VcasHashtableEpoch", "VcasHashtableSteamLF", "VcasHashtableDLRT", "VcasHashtableSLRT", "VcasHashtableBBF"]

for i in range(len(datastructures)):
  if "-param-" in datastructures[i]:
    datastructures[i] = datastructures[i] + batch_size
